...
function App() {
  ...
  useEffect(() => {
    // Better scroll reveal using IntersectionObserver
    const revealCallback = (entries: IntersectionObserverEntry[], observer: IntersectionObserver) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          entry.target.classList.add('active');
          // Once it's revealed, we don't need to observe it anymore unless we want repeat animations
          // observer.unobserve(entry.target); 
        } else {
          // Optional: Remove class when scrolled out to animate again
    ...
    // Initial check for images that might already be in viewport
  ...
  }, [view]); // Re-run when view changes as new elements might be rendered
  ...
  const navigate = (to: 'landing' | 'privacy' | 'terms' | 'dashboard') => {
    ...
    try {
      ...
      if (to === 'landing') searchParams.delete('view');
      else searchParams.set('view', to);
      const newUrl = '?' + searchParams.toString();
      window.history.pushState({}, '', to === 'landing' ? window.location.pathname : newUrl);
  ...
  return (
    <div className="min-h-screen bg-[#010101] selection:bg-blue-600/40">
      <Header isAdmin={isAdmin} onLogout={() => setIsAdmin(false)} onLoginSuccess={handleLoginSuccess} />
      ...
      <main>
        ...
        {/* ECOSYSTEM SECTION: Professional Enterprise Terminology */}
      ...
      <div className="fixed bottom-8 right-8 flex flex-col gap-4 z-[9999]">
        ...
        <button onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})} className={`w-14 h-14 glass text-white rounded-2xl shadow-2xl hover:bg-blue-600 transition-all flex items-center justify-center border border-white/10 ${showScrollTop ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
